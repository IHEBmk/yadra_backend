...
def submit_review():
    ...
    user=User.query.filter_by(id=user_id,otp_verified=True,is_hidden=False,state=0).first()
    ...
    if json_data.get("branch_id"):
        branch_id = validated_data["branch_id"]
        branch = models.Branch.query.filter_by(id=str(branch_id)).first()
        if user.role==4:
            if user.branch_id==branch_id:
                return jsonify({"msg": "You are not authorized to submit a review for this branch"}), 403
    else:
        branch_id = None
    if json_data.get("company_id"):
        company_id = validated_data["company_id"]
        company=models.Company.query.filter_by(id=str(company_id)).first()
        if user.role==3:
            if user.company_id==company_id:
                return jsonify({"msg": "You are not authorized to submit a review for this company"}), 403
    else:
        company_id = None
    ...
    if location_data:
            ...
            city = location_data.get('city', {}).get('name', 'Unknown')
    else:
            country = 'Unknown'
            city = 'Unknown'
