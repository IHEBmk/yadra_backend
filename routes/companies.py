...
def company_register_validate():
    ...
    if user and company:
        if User.query.filter_by(email=company.admin_email,is_hidden=False).first():
            return jsonify({"msg": "Admin email already exists"}), 406
        if validated:
...
@companies_blueprint.route('/get_branch_reviews', methods=['GET'])
@jwt_required()
def get_branch_reviews():
    user_id = get_jwt_identity()
    print(get_jwt_identity())
    branch_id=request.args.get("branch_id")
    branch=Branch.query.filter_by(id=branch_id,is_hidden=False).first()
    if branch:
        return jsonify({'reviews':get_branch_reviews(branch),"user_id":user_id}),200
...
@companies_blueprint.route('/company/get_branch_suggestions',methods=['POST'])
@jwt_required()
